...
# Calculate candidates' extraversion scores on the BFI --------------------
bfi <- bfi %>%
...
# Join candidates' smartphone use characteristics with their extraversion scores --------
dsuq <- dsuq %>%
...
### Generate null distribution when H0 is true
...
null_distribution_t %>%
  ...
  geom_label(x = -2.5, y = 975, label = "p-value (i.e. area in red) = 0.712", size = 5) +
  labs(x = "t-statistics")
...
## Run post-hoc test (note: This was run despite an insignificant p-value found in the ANOVA. This was just as a practice for myself.)
K <- length(unique(dsuq$X18..I.usually.use.my.smartphone.with)) *
  (length(unique(dsuq$X18..I.usually.use.my.smartphone.with)) - 1) /
  2

bonferroni_corrected_sig_lv <- 0.05 / K # Modified significance value for the post-hoc tests is 0.017
...
## between individuals who use their smartphones with left hand, right hand and two hands (p > 0.05).